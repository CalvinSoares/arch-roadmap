...
import { gerarPerguntas, sementeDoDia } from "@/shared/lib/quiz";
...
export function Quiz({
  ...
  const perguntas = useMemo(
    () => gerarPerguntas(sementeDoDia(hoje) + rodada, quantidade, escopo),
    [hoje, rodada, quantidade, escopo]
  );
  ...
  const terminou =
    perguntas.length > 0 && respostas.length >= perguntas.length && !escolha;
  ...
  /* ——— Tela final ——— */
  if (terminou) {
    const erradas = perguntas.filter((_, i) => !respostas[i]);
    ...
    return (
      <div className="rounded-2xl border border-card-border bg-card p-6">
        ...
        <p className="mt-4 text-center text-3xl font-semibold tracking-tight">
          {acertos}
          <span className="text-muted">/{perguntas.length}</span>
        </p>
        ...
        <div className="mt-5 text-center">
          {/* autoFocus: Enter emenda uma rodada na outra */}
          <Button variant="outline" size="sm" onClick={recomecar} autoFocus>
            <RotateCcw /> Outra rodada
          </Button>
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      {/* ——— Enunciado: a armadilha como citação ——— */}
      <figure className="mt-4">
        <blockquote className="border-l-2 border-primary/50 pl-3.5 text-[15px] leading-relaxed">
          {pergunta.enunciado}
        </blockquote>
        <figcaption className="mt-2.5 pl-3.5 text-[13px] font-medium text-muted">
          De qual padrão é esta armadilha?
        </figcaption>
      </figure>
