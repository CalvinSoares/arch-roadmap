 ...
 * Função pura: `hoje` entra por parâmetro, nunca `new Date()` aqui, pra spec
 ...
 * desempata por último, pra ordenação ser determinística (a spec depende).
 */
export function pontosFracos(
  ...
  for (const [slug, v] of Object.entries(d)) {
    // Checkpoints da jornada não são verbetes; ficam fora de "pontos fracos".
    if (slug.startsWith("checkpoint:") || slug.startsWith("revisao:")) continue;
